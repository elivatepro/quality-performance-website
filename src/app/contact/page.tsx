...
const inputClass =
  "w-full rounded-lg border border-border-dark bg-dark-tertiary px-4 py-3 text-sm text-text-primary placeholder:text-white/40 outline-none transition-colors duration-200 focus:border-gold focus:ring-1 focus:ring-gold";
        ...
        <div className="mx-auto max-w-[1280px] px-6 text-center lg:px-12">
          <div className="mx-auto max-w-lg">
            ...
            <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Link
                href="/"
                className="inline-flex items-center rounded-lg bg-gold px-6 py-3 text-base font-semibold text-dark transition-transform duration-200 ease-out hover:bg-gold-hover active:scale-[0.97]"
              >
                Back to Home
              </Link>
              <Link
      ...
      <section className="relative flex min-h-[38vh] items-center bg-dark">
        ...
        <div className="relative mx-auto w-full max-w-[1280px] px-6 py-28 lg:px-12">
          <div className="max-w-2xl">
            <p className="animate-hero text-sm font-semibold uppercase tracking-wider text-gold">Partner With Us</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                    ...
                    {interestOptions.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          aria-pressed={active}
                          className={`flex items-start gap-3 rounded-xl border p-3.5 text-left transition-colors duration-200 active:scale-[0.99] ${active ? "border-gold bg-gold/10" : "border-border-dark hover:border-gold/30"}`}
                        >
                          <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border transition-colors duration-200 ${active ? "border-gold bg-gold" : "border-white/25"}`}>
                            {active && (
                ...
                <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
                  <label htmlFor="wantsCall" className="flex items-start gap-3">
                    <input
                      ...
                      className="mt-0.5 h-4 w-4 rounded border-white/25 bg-transparent text-gold focus:ring-gold"
                    />
                        ...
                        <div className="flex gap-2">
                          {(["call", "text"] as const).map((pref) => (
                            <button
                              ...
                              className={`flex-1 rounded-lg border px-4 py-3 text-sm font-semibold capitalize transition-colors duration-200 active:scale-[0.98] ${contactPreference === pref ? "border-gold bg-gold/10 text-gold" : "border-border-dark text-white/60 hover:border-gold/30"}`}
                            >
                              {pref}
                            </button>
                          ))}
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="inline-flex items-center gap-2 rounded-lg bg-gold px-8 py-3 text-sm font-semibold text-dark transition-transform duration-200 ease-out hover:bg-gold-hover active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {submitting ? (
                      <>
            ...
            <AnimatedSection className="lg:col-span-2" delay={150}>
              <div className="space-y-6">
                ...
                <div className="rounded-xl border border-white/[0.08] bg-[#0D1526] p-6">
                  ...
                  <a href={`mailto:${contact.email}`} className="mt-2 block text-sm text-gold hover:underline">
                    {contact.email}
                  </a>
                ...
                <div className="rounded-xl border border-gold/15 bg-gold/[0.04] p-6">
                  ...
                  <a href={contact.dealerPortalUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-gold px-4 py-2 text-[12px] font-semibold text-gold transition-colors duration-200 hover:bg-gold hover:text-dark">
                    Dealer Portal
